...
Update the application so it displays warning with the text "Don't you think this is too much?" in
CoffeeOverview when the user orders more then 10 coffees. When the user orders at least one
coffee but no more then 10 show message "You like coffee, he?". Otherwise the message should be
"No coffee? 🤔"

Then replace three occurrences of CoffeeOverview in the App component by a simple for loop.
...
Update CoffeeOverview's HTML template and add the messages at the end of the current template
...
Create an array of coffees in App's TypeScript part
...
Use the created array in App's HTML template and loop in using `@for` block instead of three
separate elements
